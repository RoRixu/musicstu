...
class AudioController(object):
    ...
    async def play_song(self, song):
        ...
        if song.info.title == None:
            ...
            song.info.uploader_url = r.get('uploader_url')
            song.info.title = r.get('title')
            song.info.duration = parse_duration(int(r.get('duration')))
        ...
        self.guild.voice_client.play(discord.FFmpegPCMAudio(
            song.base_url, before_options='-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5', options='-vn'), after=lambda e: self.next_song(e))
        channel = self.bot.get_channel(970083365489356810)
        lastMessage = await channel.fetch_message(channel.last_message_id)
        if lastMessage.author == self.bot.user:
            await lastMessage.edit(embed=song.info.format_output(config.SONGINFO_NOW_PLAYING))
            await lastMessage.add_reaction("\U000023EF")
            await lastMessage.add_reaction("\U000023ED")
            await lastMessage.add_reaction("\U000023EE")
            await lastMessage.add_reaction("\U000023F9")
            await lastMessage.add_reaction("\U0001F509")
            await lastMessage.add_reaction("\U0001F50A")
        else:
            await channel.send(embed=song.info.format_output(config.SONGINFO_NOW_PLAYING))
            lastMessage = await channel.fetch_message(channel.last_message_id)
            await lastMessage.add_reaction("\U000023EF") #play/pause
            await lastMessage.add_reaction("\U000023ED") #next
            await lastMessage.add_reaction("\U000023EE") #prev
            await lastMessage.add_reaction("\U000023F9") #stop
            await lastMessage.add_reaction("\U0001F509") #volume down
            await lastMessage.add_reaction("\U0001F50A") #volume up
    ...
    async def process_song(self, track, ctx):
        ...
        if is_playlist != linkutils.Playlist_Types.Unknown:
            ...
            await self.process_playlist(is_playlist, track,ctx)
            ...
            song = Song(linkutils.Origins.Playlist,
                        linkutils.Sites.Unknown,requester=ctx.author,channel=ctx.channel)
            return song
        ...
        song = Song(linkutils.Origins.Default, host, base_url=r.get('url'), uploader=r.get('uploader'), title=r.get(
            'title'), duration=parse_duration(int(r.get('duration'))), webpage_url=r.get('webpage_url'), thumbnail=thumbnail, uploader_url=r.get('uploader_url'),requester=ctx.author,channel=ctx.channel)
    ...
    async def process_playlist(self, playlist_type, url,ctx):
        ...
        if playlist_type == linkutils.Playlist_Types.YouTube_Playlist:
            ...
            with yt_dlp.YoutubeDL(options) as ydl:
                ...
                for entry in r['entries']:
                    ...
                    song = Song(linkutils.Origins.Playlist,
                                linkutils.Sites.YouTube, webpage_url=link,requester=ctx.author,channel=ctx.channel)
        ...
        if playlist_type == linkutils.Playlist_Types.Spotify_Playlist:
            ...
            for link in links:
                song = Song(linkutils.Origins.Playlist,
                            linkutils.Sites.Spotify, webpage_url=link,requester=ctx.author,channel=ctx.channel)
                self.playlist.add(song)
        ...
        if playlist_type == linkutils.Playlist_Types.BandCamp_Playlist:
            ...
            with yt_dlp.YoutubeDL(options) as ydl:
                ...
                for entry in r['entries']:
                    ...
                    song = Song(linkutils.Origins.Playlist,
                                linkutils.Sites.Bandcamp, webpage_url=link,requester=ctx.author,channel=ctx.channel)
